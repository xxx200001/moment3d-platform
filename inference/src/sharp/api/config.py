...
class ServerConfig(BaseModel):
    ...
    port: int = Field(default=8000, description="服务器端�?)
...
class ModelConfig(BaseModel):
    ...
    use_gpu_postprocessing: bool = Field(default=True, description="是否�?GPU 上执行后处理（更快）")


class StorageConfig(BaseModel):
    """存储配置"""
    output_dir: str = Field(default="output/results", description="输出文件目录")
    keep_ply: bool = Field(default=False, description="压缩后是否保留原�?PLY 文件")
...
if __name__ == "__main__":
    ...
    print("配置模板已生�? config.template.json")
