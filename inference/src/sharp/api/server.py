...
async def broadcast_status(task_id: str, status: str, message: str = ""):
    ...
    Args:
        task_id: 任务 ID
        status: 状态�?(queued, converting, compressing, completed, failed)
        message: 消息（仅�?failed 状态时需要，用于传递错误信息）
    """
...
async def predict(file: UploadFile = File(...)):
    ...
    图片的焦距信息将自动�?EXIF 提取，与 CLI 工具保持一�?
...
async def stream_status(task_id: str):
    ...
    - 如果文件存在，说明任务已完成，直接返�?completed 状�?
